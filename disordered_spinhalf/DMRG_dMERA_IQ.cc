...
//    sweeps.maxm() = 10,20,50,100,200;
    ...
    ITensor Lt;                     //plain ITensor for L
    ...
    for(int i=2; i<=N-1; i++) 
    {
        ...
        for(int j=i; j<=N-1; j++)
        {
            ...
            Lt = L.toITensor();
            Lt.toMatrix22(psi.LinkInd(i-1),psi.LinkInd(j),primed(psi.LinkInd(i-1)),primed(psi.LinkInd(j)),Lm);

            //find eigenvalues and vectors
            Lm = 0.5*(Lm + Lm.t());
            EigenValues(Lm,nrg,Veig);
          
            //stabilise eigenvalues
            for(int ei=1; ei<=nrg.Length(); ei++)
            {
                if(nrg(ei) == 0)
                {
                    nrg(ei) = 1e-19;
